...
class HTelem {
	...
	child(ix) { return this.el.children[ix] }
    }
...
class SequenceHandler extends HTelem {
    constructor(el) { super(el) }
    ctr() {
        switch (this.el.tagName.toLowerCase()) {
            case 'handle': return (x) => new Handle(x);
    	    case 'index':  return (x) => new Index(x);
    	    default: throw 'invalid sequence '+ this.el.tagName;
            }
        }
    }
class Sequence extends Book {
	constructor(el, info='info') {
	    const tag= el.child(0).tagName.toLowerCase();
	    const [x0, x1] = (tag==info ? [0,1] : [1,0] )
        super (
	        { li: Array.from(el.child(x1).children)
	        , ctr: (new SequenceHandler(el.child(x1))).ctr() },
	        { topics: el.child(x0) }
            )
        this.section.bm= el.get('select') ||0;
		}
	}
class AgentTab {
